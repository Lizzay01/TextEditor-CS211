...
namespace CursesUi
{
	...
	public:
		UiComponent(WINDOW* parent = 0, int height = 0, int width = 0, int y = 0, int x = 0)
					: _canvas(parent), _height(height), _width(width), _y(y), _x(x)
		{
			if (parent != nullptr)
			{
				//								offset -
				//				WINDOW, # rows, # cols, y, x
				_canvas = derwin(parent, height, width, y, x);
			}
		}

		int getCharLocationAt(int y, int x)
		{
